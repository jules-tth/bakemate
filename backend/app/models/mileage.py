...
class MileageLog(TenantBaseModel, table=True):
    ...
    reimbursement_rate: Optional[float] = Field(
        default=None
    )  # Rate at the time of logging, can be from user settings
    reimbursement_amount: Optional[float] = Field(
        default=None
    )  # distance * reimbursement_rate
...
class MileageLogBase(SQLModel):
    ...
    reimbursement_rate: Optional[float] = (
        None  # Allow setting if not using global default
    )
