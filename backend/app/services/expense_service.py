...
class ExpenseService:
    ...
    async def create_expense(self, *, expense_in: ExpenseCreate, current_user: User, receipt_file: Optional[UploadFile] = None) -> Expense:
        ...
        if "receipt_filename" in expense_data: # Remove if it was part of Pydantic model but not for DB initially
        ...
        if receipt_file:
            ...
            file_extension = Path(receipt_file.filename).suffix if receipt_file.filename else ".dat"
            ...
            try:
                ...
                db_expense.receipt_filename = receipt_file.filename # Store original filename
                db_expense.receipt_s3_key = str(file_location) # Store path as key for local storage
                # In a real S3 setup, this would be the S3 key, and receipt_url would be S3 URL
                db_expense.receipt_url = f"/files/receipts/{saved_filename}" # Example local URL
    ...
    async def get_expense_by_id(self, *, expense_id: UUID, current_user: User) -> Optional[Expense]:
    ...
    async def update_expense(
        self, *, expense_id: UUID, expense_in: ExpenseUpdate, current_user: User, receipt_file: Optional[UploadFile] = None
    ) -> Optional[Expense]:
        ...
        if receipt_file:
            ...
            if db_expense.receipt_s3_key and Path(db_expense.receipt_s3_key).exists():
                try:
                    ...
                    print(f"Error deleting old receipt {db_expense.receipt_s3_key}: {e}")
            ...
            try:
                ...
                if hasattr(receipt_file, 'file') and receipt_file.file:
                    receipt_file.file.close()
        elif expense_in.receipt_filename is None and expense_in.receipt_s3_key is None: # Explicitly removing receipt
             if db_expense.receipt_s3_key and Path(db_expense.receipt_s3_key).exists():
                try:
                    Path(db_expense.receipt_s3_key).unlink()
                except OSError as e:
    ...
    async def delete_expense(self, *, expense_id: UUID, current_user: User) -> Optional[Expense]:
        db_expense = await self.expense_repo.get(id=expense_id)
        if not db_expense or db_expense.user_id != current_user.id:
            return None
        ...
        if db_expense.receipt_s3_key and Path(db_expense.receipt_s3_key).exists():
            try:
                ...
                print(f"Error deleting receipt file {db_expense.receipt_s3_key} during expense deletion: {e}")
    ...
    def get_receipt_file_path(self, filename: str, current_user: User) -> Optional[Path]:
