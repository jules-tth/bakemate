...
class EmailService:
    ...
    async def send_email_async(self, email_to: str, subject: str, html_content: str, email_from: str = settings.EMAIL_FROM) -> bool:
        """
    ...
    async def send_email_with_template_async(
        ...
        for key, value in environment.items():
            html_content_parts.append(f"<p><strong>{key.replace('_', ' ').title()}:</strong> {value}</p>")
        ...
        return await self.send_email_async(email_to=email_to, subject=subject, html_content=html_content)
