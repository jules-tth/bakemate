...
from app.models.contact import Contact # Assuming Contact model exists and stores customer info
from app.models.order import Order, OrderStatus # To identify top customers or dormant ones
from app.services.email_service import EmailService # For sending campaigns via SendGrid
...
class MarketingService:
    ...
    async def get_contacts_for_segment(self, segment_type: str, current_user: User) -> List[Contact]:
        ...
        if segment_type == MarketingSegment.TOP_CUSTOMERS:
            ...
            orders_subquery = select(Order.customer_email, func.count(Order.id).label("order_count"))\
                .where(Order.user_id == current_user.id, Order.status == OrderStatus.COMPLETED)\
                .group_by(Order.customer_email)\
                .having(func.count(Order.id) > 2) # Example: more than 2 orders
            
            top_customer_emails_result = self.session.exec(orders_subquery).all()
            top_customer_emails = [row.customer_email for row in top_customer_emails_result if row.customer_email]
            
            if top_customer_emails:
                contacts_stmt = select(Contact).where(Contact.user_id == current_user.id, Contact.email.in_(top_customer_emails))
            ...
            all_ordering_emails = [email for email in self.session.exec(all_ordering_customers_stmt).all() if email]

            # Find emails of customers who have ordered in the last 6 months
            recent_ordering_customers_stmt = select(Order.customer_email.distinct())\
                .where(Order.user_id == current_user.id, Order.order_date >= six_months_ago)
            recent_ordering_emails = [email for email in self.session.exec(recent_ordering_customers_stmt).all() if email]
            
            dormant_emails = list(set(all_ordering_emails) - set(recent_ordering_emails))
            
            if dormant_emails:
                contacts_stmt = select(Contact).where(Contact.user_id == current_user.id, Contact.email.in_(dormant_emails))
                contacts = self.session.exec(contacts_stmt).all()
    ...
    async def send_campaign_to_segment(
        ...
        *, 
        ...
        if not settings.SENDGRID_API_KEY or not settings.EMAILS_FROM_EMAIL:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Email service (SendGrid) is not configured.")

        contacts_to_email = await self.get_contacts_for_segment(segment_type=segment_type, current_user=current_user)
        
        if not contacts_to_email:
            return {"message": "No contacts found for the selected segment.", "sent_count": 0, "failed_count": 0}
    ...
    def get_basic_campaign_template(self, title: str, body_paragraph: str, cta_text: str, cta_url: str, shop_name: str) -> str:
