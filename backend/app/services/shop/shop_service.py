...
from app.models.shop.shop_configuration import ShopConfiguration, ShopConfigurationCreate, ShopConfigurationUpdate, ShopProduct, PublicShopView, PublicShopProductView, ShopOrderCreate, ShopOrderItemCreate, ShopStatus
from app.models.user import User
from app.models.recipe import Recipe # To fetch recipe details for shop products
from app.models.order import Order, OrderCreate, OrderItem, OrderItemCreate, OrderStatus as AppOrderStatus # Main app order status
from app.repositories.sqlite_adapter import SQLiteRepository
from app.services.order_service import OrderService # To create orders in the main system
...
class ShopService:
    def __init__(self, session: Session):
        ...
        self.order_service = OrderService(session=session) # For creating main app orders
        self.email_service = EmailService() # For notifications

    async def create_shop_configuration(self, *, shop_config_in: ShopConfigurationCreate, current_user: User) -> ShopConfiguration:
        if shop_config_in.user_id != current_user.id:
            # This should ideally be set by the system or validated strictly
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Cannot create shop configuration for another user.")
        ...
        existing_slug_stmt = select(ShopConfiguration).where(ShopConfiguration.shop_slug == shop_config_in.shop_slug)
        if self.session.exec(existing_slug_stmt).first():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Shop slug {shop_config_in.shop_slug} already exists.")

        # User can only have one shop configuration for now
        existing_shop_stmt = select(ShopConfiguration).where(ShopConfiguration.user_id == current_user.id)
        if self.session.exec(existing_shop_stmt).first():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User can only have one shop configuration. Update the existing one.")
        ...
        if shop_config_in.products:
            ...
            for prod_in in shop_config_in.products:
                ...
                if prod_in.price < 0:
                     raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Product price for {prod_in.name} cannot be negative.")
    ...
    async def get_shop_configuration_by_user(self, *, current_user: User) -> Optional[ShopConfiguration]:
        statement = select(ShopConfiguration).where(ShopConfiguration.user_id == current_user.id)
        shop_config = self.session.exec(statement).first()
        return shop_config
    
    async def get_shop_configuration_by_slug(self, *, shop_slug: str) -> Optional[ShopConfiguration]:
        statement = select(ShopConfiguration).where(ShopConfiguration.shop_slug == shop_slug)
        shop_config = self.session.exec(statement).first()
        return shop_config

    async def update_shop_configuration(self, *, shop_config_id: UUID, shop_config_in: ShopConfigurationUpdate, current_user: User) -> Optional[ShopConfiguration]:
        ...
        update_data = shop_config_in.model_dump(exclude_unset=True, exclude={"products"})
        
        if "shop_slug" in update_data and update_data["shop_slug"] != db_shop_config.shop_slug:
            existing_slug_stmt = select(ShopConfiguration).where(ShopConfiguration.shop_slug == update_data["shop_slug"])
            if self.session.exec(existing_slug_stmt).first():
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Shop slug {update_data['shop_slug']} already exists.")

        for key, value in update_data.items():
            setattr(db_shop_config, key, value)
        
        if shop_config_in.products is not None: # Allows clearing products with an empty list
            valid_shop_products = []
            for prod_in in shop_config_in.products:
                recipe = self.session.get(Recipe, prod_in.recipe_id)
                if not recipe or recipe.user_id != current_user.id:
    ...
    async def delete_shop_configuration(self, *, shop_config_id: UUID, current_user: User) -> Optional[ShopConfiguration]:
        db_shop_config = self.session.get(ShopConfiguration, shop_config_id)
        if not db_shop_config or db_shop_config.user_id != current_user.id:
            return None
    ...
    async def get_public_shop_view(self, *, shop_slug: str) -> Optional[PublicShopView]:
        ...
        if not shop_config or shop_config.status != ShopStatus.ACTIVE or not shop_config.allow_online_orders:
        ...
        for shop_product_data in shop_config.products_json or []:
            ...
            if shop_product.is_available:
                ...
                public_products.append(PublicShopProductView(
    ...
    async def create_order_from_shop(self, *, order_in: ShopOrderCreate) -> Order:
        shop_config = await self.get_shop_configuration_by_slug(shop_slug=order_in.shop_slug)
        if not shop_config or shop_config.status != ShopStatus.ACTIVE or not shop_config.allow_online_orders:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Shop not found or not accepting orders.")

        baker_user = self.session.get(User, shop_config.user_id)
        if not baker_user:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Shop owner not found.")

        order_items_create: List[OrderItemCreate] = []
        shop_products_map = {str(p['recipe_id']): ShopProduct(**p) for p in (shop_config.products_json or [])}

        for item_in in order_in.items:
            shop_product = shop_products_map.get(str(item_in.recipe_id))
            if not shop_product or not shop_product.is_available:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Product ID {item_in.recipe_id} not available or not found in shop.")
            ...
            item_cost_price = recipe_details.cost_price if recipe_details else Decimal(0) # Simplified COGS
        ...
        if shop_config.min_order_amount and total_order_amount < Decimal(shop_config.min_order_amount):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Order total is below minimum of {shop_config.min_order_amount}.")
        if shop_config.max_order_amount and total_order_amount > Decimal(shop_config.max_order_amount):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Order total exceeds maximum of {shop_config.max_order_amount}.")
        if shop_config.max_order_amount and total_order_amount > Decimal(shop_config.max_order_amount):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Order total exceeds maximum of {shop_config.max_order_amount}.")
        ...
        created_order = await self.order_service.create_order(order_in=main_order_create, current_user=baker_user, skip_stripe=True)
    ...
    async def get_embed_snippet(self, *, shop_slug: str, current_user: User) -> str:
        ...
        base_url = settings.SERVER_HOST # e.g., http://localhost:8000 or https://bakemate.app
