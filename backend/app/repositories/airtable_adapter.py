...
from sqlmodel import SQLModel # SQLModel is used for schema definition here too for consistency

from app.core.config import settings # For AIRTABLE_API_KEY, AIRTABLE_BASE_ID
from app.repositories.base import IRepository

ModelType = TypeVar("ModelType", bound=SQLModel) # Using SQLModel as base for Pydantic models
...
class AirtableRepository(IRepository[ModelType, CreateSchemaType, UpdateSchemaType], Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    def __init__(self, model: Type[ModelType], table_name: str):
        self.model = model
        self.table_name = table_name
        self.base_url = f"https://api.airtable.com/v0/{settings.AIRTABLE_BASE_ID}/{self.table_name}"
        self.headers = {
    ...
    async def _request(self, method: str, url: str, **kwargs) -> httpx.Response:
        async with httpx.AsyncClient() as client:
            try:
                ...
                print(f"Airtable API request failed: {e.response.status_code} - {e.response.text}")
    ...
    def _map_model_to_airtable_fields(self, obj_in: Union[CreateSchemaType, UpdateSchemaType, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        data.pop("airtable_record_id", None) # This is Airtable's ID, not a field to set
        return {"fields": data}
    ...
    async def get(self, *, id: str, **kwargs) -> Optional[ModelType]: # Airtable ID is a string
    ...
    async def get_multi(
        ...
        **kwargs
        ...
        current_offset: Optional[str] = kwargs.get("offset") # Allow passing initial offset
    ...
    async def delete(self, *, id: str, **kwargs) -> Optional[ModelType]: # Airtable ID is string
        url = f"{self.base_url}/{id}"
        try:
    ...
    async def get_by_attribute(
        ...
        **kwargs
        ...
        formula = f"{{{attribute_name}}}='{attribute_value}'" if isinstance(attribute_value, str) else f"{{{attribute_name}}}={attribute_value}"
