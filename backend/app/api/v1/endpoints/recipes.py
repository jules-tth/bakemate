...
async def read_recipes(
    *, 
    ...
    """
    recipe_service = RecipeService(session=session)
    recipes = await recipe_service.get_recipes_by_user(current_user=current_user, skip=skip, limit=limit)
    return recipes
...
async def read_recipe(
    *, 
    ...
    if not recipe:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Recipe not found or not owned by user")
    return recipe
...
async def delete_recipe(
    *, 
    ...
    """
    recipe_service = RecipeService(session=session)
    deleted_recipe = await recipe_service.delete_recipe(recipe_id=recipe_id, current_user=current_user)
...
@router.post("/trigger-cost-update/ingredient/{ingredient_id}", status_code=status.HTTP_202_ACCEPTED)
async def trigger_recipe_cost_updates_for_ingredient(
    *, 
    session: Session = Depends(get_session),
    ingredient_id: UUID,
    current_user: User = Depends(get_current_active_user) # Ensure only authorized users can trigger this
):
    """
    ...
    if not ingredient:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ingredient not found or not accessible by user.")

    recipe_service = RecipeService(session=session)
    await recipe_service.update_recipe_cost_on_ingredient_change(ingredient_id=ingredient_id)
    return {"msg": f"Recipe cost update process triggered for ingredient {ingredient_id}"}
