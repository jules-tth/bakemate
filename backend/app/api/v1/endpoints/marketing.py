...
async def get_segment_contacts(
    *, 
    ...
    """
    if segment_type not in [MarketingSegment.TOP_CUSTOMERS, MarketingSegment.DORMANT_CUSTOMERS]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid segment type.")

    marketing_service = MarketingService(session=session)
    contacts = await marketing_service.get_contacts_for_segment(segment_type=segment_type, current_user=current_user)
    return contacts
...
async def send_marketing_campaign(
    *, 
    ...
    """
    if campaign_data.segment_type not in [MarketingSegment.TOP_CUSTOMERS, MarketingSegment.DORMANT_CUSTOMERS]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid segment type for campaign.")
    
    if not campaign_data.subject or not campaign_data.html_content:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Campaign subject and HTML content are required.")
...
async def get_basic_campaign_template_preview(
    *, 
    session: Session = Depends(get_session), # Not strictly needed for this static template
    title: str = Body("Your Special Offer!"),
    body_paragraph: str = Body("We have something amazing just for you. Check it out now before it_s too late!"),
    cta_text: str = Body("Learn More"),
    cta_url: str = Body("https://example.com/offer"),
    current_user: User = Depends(get_current_active_user) # To get shop name or user context
):
    """
