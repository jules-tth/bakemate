...
async def adjust_ingredient_stock(
    *, 
    ...
    ingredient_id: UUID,
    quantity_change: float = Query(..., description="Amount to change stock by. Positive to add, negative to deduct."),
...
async def trigger_low_stock_check(
    *, 
    ...
    """
    inventory_service = InventoryService(session=session)
    low_stock_items = await inventory_service.run_low_stock_check_for_user(current_user=current_user)
