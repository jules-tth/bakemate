...
async def create_ingredient(
    *,
    ...
    new_ingredient = await ingredient_service.create_ingredient(
        ingredient_in=ingredient_in_corrected, current_user=current_user
    )
...
async def read_ingredients(
    *,
    ...
    """
    ingredient_service = IngredientService(session=session)
    ingredients = await ingredient_service.get_ingredients_by_user(
    ...
    *,
    ...
    if not ingredient:
        raise HTTPException(
            ...
            detail="Ingredient not found or not owned by user",
        )
    return ingredient
...
@router.delete(
    "/{ingredient_id}", response_model=IngredientRead
)  # Or just status 204 No Content
async def delete_ingredient(
    *,
    ...
    """
    ingredient_service = IngredientService(session=session)
    deleted_ingredient = await ingredient_service.delete_ingredient(
        ingredient_id=ingredient_id, current_user=current_user
    )
