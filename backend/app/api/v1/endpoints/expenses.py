...
async def create_expense(
    *,
    ...
    category_in: Optional[ExpenseCategory] = Form(
        ExpenseCategory.OTHER, alias="category"
    ),
    ...
    receipt_file: Optional[UploadFile] = File(
        None, alias="receipt"
    ),  # Alias to match form field name
    ...
    if receipt_file and receipt_file.size > 3 * 1024 * 1024:  # 3MB limit
        raise HTTPException(
            ...
            detail="Receipt file size exceeds 3MB limit.",
        )
    ...
    new_expense = await expense_service.create_expense(
        expense_in=expense_in, current_user=current_user, receipt_file=receipt_file
    )
    return new_expense
...
async def read_expense(
    *,
    ...
    expense = await expense_service.get_expense_by_id(
        expense_id=expense_id, current_user=current_user
    )
    if not expense:
        raise HTTPException(
...
async def update_expense(
    *,
    ...
    vendor_in: Optional[str] = Form(None, alias="vendor"),
    notes_in: Optional[str] = Form(None, alias="notes"),
    remove_receipt: Optional[bool] = Form(
        False, alias="remove_receipt"
    ),  # To explicitly remove receipt
    ...
    if date_in is not None:
        update_data["date"] = date_in
    if description_in is not None:
        update_data["description"] = description_in
    if amount_in is not None:
        update_data["amount"] = amount_in
    if category_in is not None:
        update_data["category"] = category_in
    if vendor_in is not None:
        update_data["vendor"] = vendor_in
    if notes_in is not None:
        update_data["notes"] = notes_in
...
async def get_receipt_file(
    ...
    ),  # Ensure user owns the expense this receipt belongs to
):
    """
    ...
    stmt = select(Expense).where(
        Expense.user_id == current_user.id, Expense.receipt_s3_key == str(file_path)
    )
    expense_record = session.exec(stmt).first()
    if not expense_record:
        raise HTTPException(
            status_code=403, detail="Receipt not found or not authorized for this user."
        )

    return FileResponse(
        path=file_path,
        filename=expense_record.receipt_filename or filename,
        media_type=(
            "image/jpeg"
