...
async def create_task(
    *, 
    ...
    new_task = await task_service.create_task(task_in=task_in, current_user=current_user)
    return new_task
...
async def read_tasks(
    *, 
    ...
    due_date_start: Optional[datetime] = Query(None, description="Filter tasks due on or after this date (ISO format)"),
    due_date_end: Optional[datetime] = Query(None, description="Filter tasks due on or before this date (ISO format)"),
...
async def delete_task(
    *, 
    ...
    """
    task_service = TaskService(session=session)
    deleted_task = await task_service.delete_task(task_id=task_id, current_user=current_user)
    if not deleted_task:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found or not owned by user")
    return deleted_task
