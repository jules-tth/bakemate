...
async def create_calendar_event(
    *,
    ...
    new_event = await calendar_service.create_calendar_event(
        event_in=event_in, current_user=current_user
    )
    return new_event
...
async def read_calendar_event(
    *,
    ...
    """
    calendar_service = CalendarService(session=session)
    event = await calendar_service.get_calendar_event_by_id(
        event_id=event_id, current_user=current_user
    )
    if not event:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Calendar event not found or not owned by user",
        )
    return event
...
async def delete_calendar_event(
    *,
    ...
    """
    calendar_service = CalendarService(session=session)
    deleted_event = await calendar_service.delete_calendar_event(
        event_id=event_id, current_user=current_user
    )
